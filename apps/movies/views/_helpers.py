...
def paginated_movies_context(request, movies_qs, page_title='home'):
    number_per_page = 20
...
def _paginated_movies(request, movies):
    return render_to_response('movies.html', paginated_movies_context(request, movies))


def _paginated_movies_string(request, movies, recommendations_qs):
    return render_to_string('movies.html', paginated_movies_context(request, movies))
